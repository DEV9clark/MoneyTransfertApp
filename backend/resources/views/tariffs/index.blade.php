...
@section('title', __('Tariff Management'))
@section('header', __('Tariff Settings'))

@section('content')
<div class="bg-white rounded-xl shadow-sm overflow-hidden mb-8">
    <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
        <h2 class="text-lg font-semibold text-gray-800">{{ __('Current Tariffs') }}</h2>
        ...
        <table class="w-full whitespace-nowrap">
            <thead>
                <tr class="bg-gray-50 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    <th class="px-6 py-3">{{ __('Min Amount') }}</th>
                    <th class="px-6 py-3">{{ __('Max Amount') }}</th>
                    <th class="px-6 py-3">{{ __('Type') }}</th>
                    <th class="px-6 py-3">{{ __('Fee Amount') }}</th>
                    @if(auth()->user()->role === 'admin')
            ...
            <tbody class="divide-y divide-gray-100" id="tariff-table-body">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button onclick='editTariff(@json($tariff))' class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('Edit') }}</button>
                        <button onclick="deleteTariff({{ $tariff->id }})" class="text-red-600 hover:text-red-900">{{ __('Delete') }}</button>
                    </td>
                    @endif
                </tr>
                @endforeach
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">{{ __('Tariff Details') }}</h3>
                <form id="tariffForm" class="mt-4 space-y-4">
                    @csrf
                    <input type="hidden" id="tariffId" name="id">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Min Amount') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Max Amount') }}</label>
                            <input type="number" id="max_amount" name="max_amount" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm border p-2">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">{{ __('Transaction Type') }}</label>
                        <select id="type" name="type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm border p-2">
                            <option value="send">{{ __('Send Money') }}</option>
                            <option value="withdraw">{{ __('Withdraw Money') }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">{{ __('Fee Amount') }} (XOF)</label>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" onclick="saveTariff()" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-indigo-700 sm:ml-3 sm:w-auto sm:text-sm">{{ __('Save') }}</button>
                <button type="button" onclick="closeTariffModal()" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">{{ __('Cancel') }}</button>
            </div>
        </div>
    </div>
</div>
